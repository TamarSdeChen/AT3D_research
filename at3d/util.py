...
def load_2parameter_lwc_file(file_name, density='lwc'):
    ...
    nx, ny, nz = np.fromstring(header['2 parameter LWC file'][0], sep=' ').astype(np.int32)
    dx, dy = np.fromstring(header['2 parameter LWC file'][1], sep=' ').astype(np.float32)
    z = np.fromstring(header['2 parameter LWC file'][2], sep=' ').astype(np.float32)
    temperature = np.fromstring(header['2 parameter LWC file'][3], sep=' ').astype(np.float32)
    ...
    i, j, k = data[:, 0].astype(np.int32)-1, data[:, 1].astype(np.int32)-1, data[:, 2].astype(np.int32)-1
    lwc[i, j, k] = data[:, 3]
    reff[i, j, k] = data[:, 4]
