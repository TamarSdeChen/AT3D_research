...
from shdom.CloudCT_setup import plank
...
def main(cloud_indices):
    ...
    run_params = load_run_params(params_path="configs/params_fixed_imagers.yaml")
    ...
    for cloud_index in cloud_indices:
        # filename1 = os.path.join(run_params['neural_network']['satellites_images_path'],'train',
        #                         f'cloud_results_{cloud_index}.pkl')
        # filename2 = os.path.join(run_params['neural_network']['satellites_images_path'],'test',
        #                         f'cloud_results_{cloud_index}.pkl')
        ...
        # if np.sum(droplets.get_extinction(wavelengths_micron[0]).data > 1) < 50:
        #     continue
        # -----------------------------------------------
        ...
        if run_params['DOFORWARD']:
            ...
            if forward_options['SEE_IMAGES']:
                ...
                plt.show()
            cameras_R = np.array([camera._rotation_matrix for camera in CloudCT_measurements.setup[0].projection_list])
            cameras_k = np.array([camera._k for camera in CloudCT_measurements.setup[0].projection_list])
            cameras_P = np.array([camera.projection_matrix for camera in CloudCT_measurements.setup[0].projection_list])
            cameras_pos = np.array([camera.position for camera in CloudCT_measurements.setup[0].projection_list])
            ext = droplets.get_extinction(wavelength).data
            grid = np.array([droplets_grid.x, droplets_grid.y, droplets_grid.z])
            # save images as mat for cloudCT neural network
            images = CloudCT_measurements.images[0]
            measurements = shdom.Measurements(shdom.Camera(sensor=shdom.RadianceSensor(),projection=shdom.MultiViewProjection(
                CloudCT_measurements.setup[0].projection_list)),images=images)
            carver = shdom.SpaceCarver(measurements)

            mask70 = carver.carve(droplets_grid, agreement=0.7, thresholds=run_params['radiance_threshold'])
            mask80 = carver.carve(droplets_grid, agreement=0.8, thresholds=run_params['radiance_threshold'])
            mask90 = carver.carve(droplets_grid, agreement=0.9, thresholds=run_params['radiance_threshold'])



            cloud = {'images': np.array(CloudCT_measurements.images[0]),
                      'cameras_R': cameras_R,
                      'cameras_k': cameras_k,
                     ...
                     'mask0.8': mask80.data,
                     'mask0.9': mask90.data,
                     'mask_morph': (ndimage.morphology.binary_closing(mask70.data, np.ones((3, 3, 3)))).astype(bool)
                      }
...
def setup_imager(imager_options, run_params, MieTablesPath, simple_type):
    ...
    imager_config = run_params['SATS_NUMBER_SETUP'] * [True]
...
if __name__ == '__main__':
    run_params = load_run_params(params_path="configs/params_fixed_imagers.yaml")
    clouds_path = run_params['CloudFieldFile'].replace('{CLOUD_INDEX}', '*')
    satellites_images_indices = [i.split('/')[-1].split('cloud')[1].split('.txt')[0] for i in
